  ...
  (define convmap `(
      (,(db "tgroup") *preorder* . ,(lambda node
              ...
              (let* ((ncols (sxml:number
                  ...
                  `(env calstable
                      (cmd TxColwidths (gr ,@(map (lambda (col)
                        `(gr ,(if (car col)
                                `(cmd f (wr nonl2) (gr ,(car col) ,(cadr col)))
                                '())
                             ,(if (caddr col)
                                `(cmd p (wr nonl2) (gr ,(caddr col)))
                                '())))
                                                  colwidths)))
                      ,@(pre-post-order (cdr node) convmap-ref)))))
      (,(db "thead")  . ,(lambda (_ . kids) `(cmd TxThead (gr ,@kids))))
      (,(db "tfoot")  . ,(lambda (_ . kids) `(cmd tfoot (gr ,@kids))))
